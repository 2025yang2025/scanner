...
# ==============================================================================
# 🌐 【美股全自動動態獲取】標普 500 + 納斯達克 100 成分股總表 (約 500+ 檔精華)
# ==============================================================================
def fetch_all_us_market_tickers():
    """動態從 Wikipedia 抓取 S&P 500 與 Nasdaq 100 最新成分股，免手工維護"""
    us_tickers = set()
    ...
    try:
        ...
        print(f"⚠️ 抓取美股代碼時發生波動: {e}，啟用核心巨頭保底。")
...
def inspect_us_earnings_filter(ticker_symbol):
    """
    動態下載財報，【只回傳】營收與淨利雙雙成長(QoQ > 0)的精選績優股
    """
    try:
        ...
        # 💡 【核心篩選門檻】：必須營收與淨利雙雙大於 0% (增長) 才放行
        if rev_qoq > 0 and net_qoq > 0 and net_latest > 0:
            rev_billion = rev_latest / 1e9
            return f"• `{ticker_symbol}`: 營收 `{rev_billion:.1f}B` (📈 `{rev_qoq:+.1f}%` QoQ) | 淨利 (🟢 `{net_qoq:+.1f}%` QoQ)"
...
if __name__ == "__main__":
    ...
    if current_hour < 11:
        if os.path.exists("results.md"):
            ...
            remind_msg = saved_content.replace("# 📊 *全球市場雙增長與策略選股報告*", "🔔 *【開盤前提醒】雙市場精選報告*")
            send_telegram_message(remind_msg)
            print("✅ 晨間提醒流程執行完畢！")
        exit(0)

    # 📊 下午盤後時段
    print("🚀 啟動【全球市場雙增長與策略選股系統】...")
    
    # --- Part 1: 台股全市場篩選 (格式完美支援中文名稱) ---
    ...
    # --- Part 2: 美股全市場大市值篩選 (只留下成長股，分批安全防禦) ---
    ALL_US_TICKERS = fetch_all_us_market_tickers()
    
    # 💡 為了避免美股伺服器偵測封鎖，我們從 500 檔中挑選與台灣供應鏈連動最深的「科技、半導體與核心製造」前 80 檔大市值權值進行精細財報健檢
    tech_keywords = ["NVDA", "AAPL", "MSFT", "AMZN", "META", "GOOGL", "AMD", "AVGO", "TSM", "SMCI", "ASML", "QCOM", "INTC", "PANW", "MU", "LRCX", "AMAT", "ADI", "TXN", "NXPI"]
    us_scan_pool = [tk for tk in ALL_US_TICKERS if tk in tech_keywords or any(tk.startswith(letter) for letter in ['A', 'M', 'N', 'T'])]
    us_scan_pool = sorted(list(set(us_scan_pool[:75]))) # 嚴格控制連線數，極速且安全
    
    print(f"⏳ 正在安全分批掃描全美精選科技權值財報 (共 {len(us_scan_pool)} 檔)...")
    us_growth_reports = []
    for idx, us_tk in enumerate(us_scan_pool, 1):
        if idx % 15 == 0:
            time.sleep(random.uniform(2.5, 4.0)) # 隨機防禦延遲
        report_line = inspect_us_earnings_filter(us_tk)
        if report_line: # 💡 只有回傳不為空值（符合雙增長）的股票才列入
            us_growth_reports.append(report_line)
        time.sleep(0.5)

    # 📝 格式化全球雙市場綜合報告
    tw_time_str = now_tw.strftime('%Y-%m-%d %H:%M:%S')
    tg_msg = f"# 📊 *全球市場雙增長與策略選股報告*\n⏰ 執行時間: {tw_time_str}\n"
    tg_msg += "---"
    
    # 🇺🇸 美股雙增長專區 (全新改變！)
    tg_msg += "\n\n🇺🇸 *【美股全市場：最新季度財報雙增長績優股】* 🚀\n"
    tg_msg += "↳ *過濾條件*：全美核心成分股中，最新單季營收與淨利『雙雙 QoQ 正成長』之企業（衰退者已自動隱藏）。\n"
    ...
    print("✅ 全球雙市場全自動安全篩選（含美股雙增長過濾）順利完成！")
